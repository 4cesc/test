...
#endif
CPicture::CPicture()  :m_pPic(NULL), _h_him(0), _w_him(0), m_size(0,0){}
CPicture::~CPicture() 
{ 
	Release();
}
...
{ 
	...
	{ 
		...
		pDC->DPtoHIMETRIC(&szOrig);   pDC->DPtoHIMETRIC(&szSrc); 
		m_pPic->Render(*pDC, lprcDest->left,lprcDest->top,lprcDest->right-lprcDest->left, 
			lprcDest->bottom-lprcDest->top, szOrig.cx, _h_him-szOrig.cy, szSrc.cx, -szSrc.cy, NULL); 
	} 
}
